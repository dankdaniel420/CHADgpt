...
class WorldSolverMultiList:
    ...
    def __init__(self, word_list_file_paths: list = [], word_length : int = 5, exclude_plurals:bool=True, word_socres:dict={}, order_words_by_score_desc:bool=False, word_symbol_combinations:dict={}):
        ...
        for file_path in self.word_list_file_paths:
            word_list = utility.Utility.load_word_list(file_path, self.word_length, self.exclude_plurals)
...
class WordleSolver():
    ...
    def __init__(self, word_list_file_path: str = None, word_length : int = 5, exclude_plurals:bool=False):
        self.permitted_input_symbols = "ox-?"
        self.word_list = []
        self.word_socres = {}
        ...
        if self.word_list_file_path is not None:
            self.word_list = utility.Utility.load_word_list(self.word_list_file_path, self.word_length, self.exclude_plurals)
        self.reset()
        ...
        for (word, symbol_pattern) in self.tries:
            for i in range(0, self.word_length):
                ...
                if symbol_pattern[i] == "?":
                    ...
                    pattern_list = list(symbol_pattern)
                    if letter == right_spot_pattern_list[i]:
                        pattern_list[i] = "o"
                    elif letter in self.wrong_spot_pattern[i]:
                        pattern_list[i] = "x"
                    elif letter in self.excluded_letters:
                        pattern_list[i] = "-"
                    symbol_pattern = ''.join(pattern_list)
                    
                if symbol_pattern[i] in ["o", "x"]:
                    self.included_letters += word[i]
                    if symbol_pattern[i] == "o":
                        right_spot_pattern_list = list(self.right_spot_pattern)
                        right_spot_pattern_list[i] = word[i]
                        self.right_spot_pattern = "".join(right_spot_pattern_list)
                    elif symbol_pattern[i] == "x":
...
if __name__ == "__main__":
    ...
    argParser.add_argument("--plurals", help="Do not exclude plurals when loading from the word list", required=False, default=True, type=bool)
    ...
    while True:
        ...
        if ":" in user_input:
            ...
            if not all([len(value) == solver_multi.word_length for value in values]):
                ...
                continue
            if values[1] == "o" * solver_multi.word_length:
                print("Great!")
                reset = True
            else:
